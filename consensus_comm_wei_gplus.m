function [S2, Q2, X_new3, qpc] = consensus_comm_wei_gplus(C,gplusValues)
...
for gs = 1:numel(gplusValues)
    gs
    for i = 1:npart;
        [S2{gs}(i,:) Q2{gs}(i)] = multislice_static_unsigned(X_new3,gplusValues(gs));
    end
    qpc(gs) = sum(sum(abs(diff(S2{gs}))));
end
